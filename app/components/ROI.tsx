      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 relative">
        ...
        <AnimateIn animation="fade-in-up" delay={100}>
          <div className="grid grid-cols-2 lg:grid-cols-4 grid-rows-auto gap-3 mb-6">
            ...
            {/* Big stat 1 — full width on mobile, 2 cols on lg */}
            <div
              className="col-span-2 row-span-1 lg:col-span-2 lg:row-span-2 rounded-2xl border border-dark-border bg-dark-card p-5 sm:p-7 flex flex-col justify-between relative overflow-hidden group neon-card"
            ...
            >
              ...
              <div>
                <p className="text-xs text-gray-600 uppercase tracking-widest mb-2 sm:mb-3">Tiempo ahorrado</p>
                <div className="font-display font-black gradient-text leading-none mb-2" style={{ fontSize: 'clamp(2.5rem, 10vw, 5rem)', textShadow: '0 0 40px rgba(124,58,237,0.4)' }}>8–15h</div>
                <div className="text-sm sm:text-base font-medium text-gray-300">por semana, por cliente</div>
              </div>
              <p className="text-xs sm:text-sm text-gray-600 mt-3 sm:mt-4">En promedio, automatizando solo el proceso principal del negocio.</p>
            </div>

            {/* Stat 2 — neon cyan */}
            <div className="col-span-1 rounded-2xl border border-dark-border bg-dark-card p-4 sm:p-5 flex flex-col justify-between relative overflow-hidden neon-card neon-card-cyan group">
              ...
              <p className="text-[10px] sm:text-xs text-gray-600 uppercase tracking-widest mb-2">Respuesta leads</p>
              <CountUp
                to={60}
                prefix="+"
                suffix="%"
                className="font-display text-3xl sm:text-5xl font-black text-brand-cyan leading-none"
                style={{ textShadow: '0 0 30px rgba(6,182,212,0.5)' }}
              />
              <p className="text-[10px] sm:text-xs text-gray-600 mt-2">tasa de contacto</p>
            </div>

            {/* Stat 3 */}
            <div className="col-span-1 rounded-2xl border border-dark-border bg-dark-card p-4 sm:p-5 flex flex-col justify-between relative overflow-hidden neon-card group">
              <div className="absolute top-0 left-0 right-0 h-[1.5px] rounded-t-2xl opacity-60 group-hover:opacity-100 transition-opacity"
                style={{ background: 'linear-gradient(90deg, transparent, #9F67FF, transparent)' }} />
              <p className="text-[10px] sm:text-xs text-gray-600 uppercase tracking-widest mb-2">Tiempo respuesta</p>
              <div className="font-display text-3xl sm:text-5xl font-black gradient-text leading-none" style={{ textShadow: '0 0 30px rgba(124,58,237,0.4)' }}>&lt;2 min</div>
              <p className="text-[10px] sm:text-xs text-gray-600 mt-2">vs 2–4h manual</p>
            </div>

            {/* Stat 4 — ROI, neon featured */}
            <div className="col-span-2 rounded-2xl border border-brand-purple/40 bg-gradient-to-br from-brand-purple/12 to-dark-card p-4 sm:p-5 flex items-center gap-4 sm:gap-5 relative overflow-hidden neon-card group"
              ...
              <CountUp
                ...
                style={{ fontSize: 'clamp(2rem, 8vw, 4rem)', textShadow: '0 0 40px rgba(124,58,237,0.5)' }}
              />
              <div>
                <p className="font-display text-sm sm:text-base font-bold text-white tracking-tight">ROI en los primeros 90 días</p>
                <p className="text-xs sm:text-sm text-gray-500 mt-1">Setup recuperado en un trimestre</p>
              </div>
              {/* Shimmer */}
        ...
        <AnimateIn animation="fade-in-up" delay={300}>
          <div className="rounded-2xl border border-brand-purple/20 bg-gradient-to-r from-brand-purple/8 via-dark-card to-brand-cyan/8 p-5 sm:p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5 sm:gap-6 relative overflow-hidden">
